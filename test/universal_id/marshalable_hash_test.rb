...
class UniversalID::MarshalableHashTest < ActiveSupport::TestCase
  def setup
    @hash = {
      test: true,
      example: "value",
  ...
  def test_config
    ...
    assert expected, UniversalID::MarshalableHash.config
  end

  def test_marshalable_hash
    assert @hash, @marshalable_hash.to_h
  end
  ...
  def test_to_packable
    ...
    assert @marshalable_hash.to_packable(except: %w[created_at updated_at remove]).is_a?(UniversalID::PackableObject)
    assert_equal expected, @marshalable_hash.to_packable(except: %w[created_at updated_at remove]).object
  end

  def test_to_packable_cache_key
    expected = "UniversalID::PackableObject/c0e15c990791eaa8c00b71a89444e4f2"
    actual = @marshalable_hash.to_packable(except: %w[created_at updated_at remove]).cache_key
    assert_equal expected, actual
  end

  def test_to_packable_id
    expected = "eNprXlKSWlxyeHlqRWJuQU7q0rLEnNLUZXlAsdSUxiXZqakFYAIAdhkSPA"
    actual = @marshalable_hash.to_packable(except: %w[created_at updated_at remove]).id
    assert_equal expected, actual
  end

  def test_find_by_id
    id = @marshalable_hash.to_packable(except: %w[created_at updated_at remove]).id
    hash = UniversalID::MarshalableHash.find(id)
    expected = {
      "test" => true,
      "example" => "value",
      "nested" => {"keep" => "keep"}
